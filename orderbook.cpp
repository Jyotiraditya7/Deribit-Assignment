...
public:
    ...
    void on_message(client* c, websocketpp::connection_hdl hdl, client::message_ptr msg) {
        ...
        if (response["id"].get<int>() == 9929) {
            ...
            // Send the order book request message
            c->send(hdl, orderbook_msg, websocketpp::frame::opcode::text);
        }
        else if (response["id"].get<int>() == 8772) {
            std::cout << "Order book response received: " << std::endl;
            std::cout << response.dump(4) << std::endl;  // Print JSON with indentation
            m_done = true;  // Close after receiving the order book response
        }
    }
