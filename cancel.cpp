...
public:
    ...
    void on_message(client* c, websocketpp::connection_hdl hdl, client::message_ptr msg) {
        ...
        if (response["id"].get<int>() == 9929) {
            ...
            c->send(hdl, cancel_msg, websocketpp::frame::opcode::text);
        }
        else if (response["id"].get<int>() == 4122) {
            std::cout << "Cancel order response received: " << std::endl;
            std::cout << response.dump(4) << std::endl;  // Print JSON with indentation
            m_done = true;  // Close after receiving the cancel response
        }
    }
