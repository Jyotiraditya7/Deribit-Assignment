...
std::string auth_msg = R"({
  ...
  "params" : {
    ...
    "client_id" : "3LMbP1Fr",
    "client_secret" : "Irv-RdIqJEiHYeg6qq-giKKqthrSDhx7HRD14hih_Cs"
  }
})";

std::string modify_msg = R"({
  "jsonrpc": "2.0",
  "id": 3725,
  "method": "private/edit",
  "params": {
    "order_id": "ETH-14405467908",
    "amount": 4,
    "price": 222
  }
})";
...
public:
    ...
    void on_message(client* c, websocketpp::connection_hdl hdl, client::message_ptr msg) {
        ...
        if (response["id"].get<int>() == 9929) {
            ...
            std::cout << response.dump(4) << std::endl;  // Print JSON with indentation

            // Send the modify order message
            c->send(hdl, modify_msg, websocketpp::frame::opcode::text);
        }
        else if (response["id"].get<int>() == 3725) {
            std::cout << "Modify order response received: " << std::endl;
            std::cout << response.dump(4) << std::endl;  // Print JSON with indentation
            m_done = true;  // Close after receiving the modify response
        }
    }
