...
public:
    ...
    void on_message(client* c, websocketpp::connection_hdl hdl, client::message_ptr msg) {
        ...
        if (response["id"].get<int>() == 9929) {
            ...
            // Send the message
            c->send(hdl, currpos_msg, websocketpp::frame::opcode::text);
        }
        else if (response["id"].get<int>() == 2236) {
            std::cout << "Current position response received: " << std::endl;
            std::cout << response.dump(4) << std::endl;  // Print JSON with indentation
            m_done = true;  // Close after receiving the response
        }
    }
